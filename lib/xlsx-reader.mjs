...
export async function readXlsxRows(file, preferredSheet = '品牌招商资料') {
  ...
  const relationships = new Map(Array.from(relationshipsXml.getElementsByTagName('Relationship')).map(item => [item.getAttribute('Id'), item.getAttribute('Target')]));
  ...
  const sharedStrings = sharedText ? Array.from(parseXml(sharedText).getElementsByTagName('si')).map(nodeText) : [];
