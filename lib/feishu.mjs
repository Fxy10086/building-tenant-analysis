...
async function collectOptionLabels(config, token, tableId) {
  const tables = await listTables(config, token);
  const tableIds = [...new Set([tableId, ...tables.map(table => table.table_id).filter(Boolean)])];
...
export async function syncFeishuSalesTable() {
  ...
  const [sourceRecords, labels] = await Promise.all([
    listRecords(config, token, table.table_id),
    collectOptionLabels(config, token, table.table_id)
  ]);
