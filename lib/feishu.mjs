...
const PAGE_SIZE = 500;
const SALES_TABLES = Object.freeze({
  '2025': { tableId: 'tblgu08zG3aNQ6Q3', tableName: '2025\u5e74\u9500\u552e\u62a5\u8868' },
  '2026': { tableId: 'tblvs4daGADrz2cu', tableName: '2026\u5e74\u9500\u552e\u62a5\u8868' }
});
...
function salesTableConfig(year) {
  const selectedYear = SALES_TABLES[String(year)] ? String(year) : '2026';
  const selected = SALES_TABLES[selectedYear];
  return {
    year: selectedYear,
    tableId: configValue(`FEISHU_TABLE_ID_${selectedYear}`) || selected.tableId,
    tableName: configValue(`FEISHU_TABLE_NAME_${selectedYear}`) || selected.tableName
  };
}
...
function referencedTableIds(records) {
  const ids = new Set();
  const visit = value => {
    if (Array.isArray(value)) return value.forEach(visit);
    if (!value || typeof value !== 'object') return;
    if (value.table_id) ids.add(value.table_id);
    Object.values(value).forEach(visit);
  };
  records.forEach(record => visit(record.fields));
  return [...ids];
}

async function collectOptionLabels(config, token, tableId, records) {
  const tableIds = [...new Set([tableId, ...referencedTableIds(records)])];
  const results = await Promise.allSettled(tableIds.map(id => listFields(config, token, id)));
  return optionLabels(results.flatMap(result => result.status === 'fulfilled' ? result.value : []));
}

export async function syncFeishuSalesTable(year = '2026') {
  const config = getFeishuConfig();
  const salesTable = salesTableConfig(year);
  ...
  let table = { table_id: salesTable.tableId, name: salesTable.tableName };
  if (!salesTable.tableId) {
    const tables = await listTables(config, token);
    table = tables.find(item => item.name === salesTable.tableName);
    if (!table?.table_id) {
      const error = new Error(`Feishu table not found: ${salesTable.tableName}`);
      error.status = 404;
      throw error;
    }
  }
  const sourceRecords = await listRecords(config, token, table.table_id);
  const labels = await collectOptionLabels(config, token, table.table_id, sourceRecords);
  const records = sourceRecords.map(record => normalizeFeishuRecord(record, labels));
  return {
    year: salesTable.year,
...
export { FIELD_NAMES, SALES_TABLES };
