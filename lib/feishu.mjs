...
export function getFeishuConfig() {
  return {
    ...
    tableName: configValue('FEISHU_TABLE_NAME') || '\u5546\u4e1a\u9500\u552e\u6570\u636e\u5206\u6790'
  };
}
...
export function normalizeFeishuRecord(record = {}) {
  ...
  return {
    ...
    floorArea: scalar(fieldValue(fields, FIELD_NAMES.floorArea)),
    floor: scalar(fieldValue(fields, FIELD_NAMES.floor)),
    unit: scalar(fieldValue(fields, FIELD_NAMES.unit)),
    category: scalar(fieldValue(fields, FIELD_NAMES.category)),
    subcategory: scalar(fieldValue(fields, FIELD_NAMES.subcategory)),
...
export async function syncFeishuSalesTable() {
  ...
  const table = tables.find(item => item.name === config.tableName);
  if (!table?.table_id) {
  ...
  const records = sourceRecords.map(normalizeFeishuRecord);
  return {
