...
const migrateSavedCategory = item => {
  const migrated=item.group==='饮品'?{...item,group:'餐饮',subtype:item.subtype||'饮品'}:item.group==='健身'?{...item,group:'休闲娱乐',subtype:'休闲娱乐'}:item.group==='百货'?{...item,group:'零售',subtype:'零售'}:item;
  const category=inferPoiCategory({name:migrated.merchant||migrated.name,type:migrated.amapType,typecode:migrated.amapTypecode});
  return {...migrated,group:category.group,subtype:category.subtype,classificationStatus:category.status,classificationReason:category.reason,classificationConfidence:category.confidence,classificationSource:category.source};
};
...
export default function Workbench() {
    ...
    const timer=setTimeout(()=>{
      ...
      fetch(`/api/amap?${query}`,{signal:controller.signal})
        ...
        .then(data=>{
          ...
          const items=externalPois.map(poi=>{const category=inferPoiCategory(poi);return {id:`amap-${poi.id}`,key:null,merchant:poi.name,branch:poi.mall?`${poi.mall}内`:'高德门店',mall:poi.mall||'',group:category.group,subtype:category.subtype,classificationStatus:category.status,classificationReason:category.reason,classificationConfidence:category.confidence,classificationSource:category.source,amapType:poi.type,amapTypecode:poi.typecode,address:poi.address,distance:poi.distance,rating:poi.rating||'暂无',sales:poi.cost?`人均 ${poi.cost} 元`:'真实 POI',x:null,y:null,poiId:poi.id,lng:poi.lng,lat:poi.lat};});
  ...
  const saveLocationCandidate = item => {
    ...
    setSavedPoiCandidates(current=>[...current,{...item,source:'高德地图',stage:'待补充资料',owner:'待分配',next:item.classificationStatus==='review'?'审核业态分类':'补充面积、租金与品牌联系人'}]);
  ...
  function AnalysisPage(){
    ...
    if(selectedPoi?.classificationStatus==='review') {
      return <>{header(<span className="badge warn">业态待审核</span>)}<div className="analysis-grid"><section className="panel"><div className="panel-head"><div><h2>{selectedPoi.merchant}</h2><p className="page-subtitle">{selectedPoi.address}</p></div><span className="badge warn">暂不生成分析结论</span></div><div className="form-row"><label className="field"><span className="field-label">当前大类判断</span><input className="form-control" value={selectedPoi.group} readOnly/></label><label className="field"><span className="field-label">当前细分判断</span><input className="form-control" value={selectedPoi.subtype} readOnly/></label></div><div className="integration-notice warn"><strong>分类依据不足</strong><span>{selectedPoi.classificationReason||'高德名称和类型不足以确认业态。'}</span></div><div className="button-row"><button className="btn primary" onClick={()=>openSupplementDialog({type:'poi',poiId:selectedPoi.poiId,name:selectedPoi.merchant})}><Plus size={15}/>补资料并确认业态</button><Link href="/shortlist" className="btn">返回候选库</Link></div></section><section className="panel"><div className="panel-head"><h2>审核要求</h2><span className="panel-note">不使用默认分类</span></div><div className="decision-grid"><div className="decision"><h3>可保留的信息</h3><p>门店名称、地址、距离和高德评分仍可用于品牌接洽。</p></div><div className="decision risk"><h3>暂停的信息</h3><p>在业态确认前，不计算楼内同业供给、分流、铺位适配和招商结论。</p></div></div></section></div></>;
    }
    if(selectedPoi) {
  ...
  const classificationReviewCount=searchResults.filter(item=>item.classificationStatus==='review').length;
  const selectedLocations=selected.map(id=>activeLocations.find(item=>item.id===id)).filter(Boolean);
  const toggleLocation=id=>{
    const location=activeLocations.find(item=>item.id===id);
    if(location?.classificationStatus==='review') return notify(`${location.merchant} 的业态需要先审核`);
    setSelected(current=>current.includes(id)?current.filter(item=>item!==id):current.length>=3?(notify('最多选择 3 家候选门店'),current):[...current,id]);
  };

  useEffect(()=>{
    setSelected(current=>current.filter(id=>activeLocations.some(item=>item.id===id&&item.classificationStatus!=='review')));
  },[activeLocations]);
  ...
  // These page helpers close over the workbench state. Render them as stable
  // element trees instead of recreating nested component types on each state
  // update, which would otherwise remount the search input after every key.
