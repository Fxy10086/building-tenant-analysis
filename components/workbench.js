...
export default function Workbench() {
  ...
  const analysisFileInput = useRef(null);
  ...
  const uploadAnalysisWorkbook = async event => {
    ...
    try {
      ...
      setAnalysisSupplement({...parsed.data,completeness:parsed.completeness,fileName:file.name});
  ...
  const analysisUploadPanel = () => <div className={`analysis-upload ${analysisUploadState==='error'?'error':''}`}><div className="analysis-upload-copy"><FileSpreadsheet size={19}/><div><strong>补充分析资料</strong><span>{analysisUploadState==='ready'?`${analysisSupplement.name} · 已生成报告`:analysisUploadState==='reading'?'正在校验 Excel...':'可下载空白模板，或直接使用已填写示例'}</span></div></div><div className="analysis-upload-actions"><a className="btn" href="/templates/候选商户补充分析资料模板.xlsx" download><Download size={15}/>空白模板</a><a className="btn" href="/templates/胖子龙虾·烧烤·小海鲜(中关村店)-补充分析资料示例.xlsx" download><FileSpreadsheet size={15}/>填写示例</a><button className="btn primary" onClick={()=>analysisFileInput.current?.click()} disabled={analysisUploadState==='reading'}><Upload size={15}/>{analysisUploadState==='reading'?'读取中':'上传 Excel'}</button>{analysisSupplement&&<button className="icon-btn danger" onClick={clearAnalysisSupplement} aria-label="移除上传资料" title="移除上传资料"><Trash2 size={15}/></button>}<input ref={analysisFileInput} type="file" accept=".xlsx,application/vnd.openxmlformats-officedocument.spreadsheetml.sheet" onChange={uploadAnalysisWorkbook} style={{display:'none'}}/></div>{analysisUploadError&&<p>{analysisUploadError}</p>}<small>文件仅在当前浏览器中解析，不上传服务器、不回写飞书。</small></div>;
  ...
  function AnalysisPage(){
    ...
    if(selectedPoi) return <>{header(<button className="btn" onClick={()=>setDrawer('history')}><History size={16}/>分析历史</button>)}
      <div className="analysis-grid"><div className="grid"><section className="panel"><div className="panel-head"><div><h2>选择分析对象</h2><p className="page-subtitle">显示候选库中的全部商户</p></div><Link href="/shortlist" className="panel-note">管理候选库</Link></div>
        ...
        <div className="button-row"><Link className="btn" href="/shortlist"><Plus size={16}/>编辑候选资料</Link><Link className="icon-btn" href="/shortlist" aria-label="管理候选商户库" title="管理候选商户库"><BookmarkCheck size={16}/></Link></div>{analysisUploadPanel()}</section>
    ...
    return <>{header(<button className="btn" onClick={()=>setDrawer('history')}><History size={16}/>分析历史</button>)}
      <div className="analysis-grid"><div className="grid"><section className="panel"><div className="panel-head"><div><h2>选择分析对象</h2><p className="page-subtitle">仅显示已加入候选商户库的品牌</p></div><Link href="/shortlist" className="panel-note">管理候选库</Link></div>
        ...
        <div className="button-row"><button className="btn primary" onClick={()=>notify(`${p.name} 的招商分析已更新`)}><Sparkles size={16}/>生成招商分析</button><Link className="icon-btn" href="/shortlist" aria-label="管理候选商户库" title="管理候选商户库"><BookmarkCheck size={16}/></Link></div>{analysisUploadPanel()}</section>
