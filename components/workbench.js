  ...
  function AnalysisPage(){
    ...
    if(selectedPoi) return <>{header(<button className="btn" onClick={()=>setDrawer('history')}><History size={16}/>分析历史</button>)}
      <div className="analysis-grid"><div className="grid"><section className="panel"><div className="panel-head"><div><h2>选择分析对象</h2><p className="page-subtitle">显示候选库中的全部商户</p></div><Link href="/shortlist" className="panel-note">管理候选库</Link></div>
        ...
        <div className="form-row"><label className="field"><span className="field-label">业态</span><input className="form-control" value={selectedPoi.group||'待确认'} readOnly/></label><label className="field"><span className="field-label">高德评分</span><input className="form-control" value={selectedPoi.rating||'暂无'} readOnly/></label><label className="field"><span className="field-label">距写字楼</span><input className="form-control" value={formatDistance(selectedPoi.distance)} readOnly/></label><label className="field"><span className="field-label">经营参数</span><input className="form-control" value="待补充" readOnly/></label></div>
        ...
        <section className="panel"><div className="result-head"><div><h2>{selectedPoi.merchant} · 初步招商分析</h2><div className="badge-row"><span className="badge blue">高德真实商户</span><span className="badge warn">待补充经营资料</span></div><p className="summary">该商户已从高德地图加入候选库。位置与业态信息可用于初步判断，完成品牌、面积、租金和经营数据后生成完整招商评分。</p></div><span className="panel-note">{selectedPoi.address}</span></div>
          <div className="section"><div className="decision-grid"><div className="decision"><h3>已具备信息</h3><p>{selectedPoi.group||'业态待确认'} · 距写字楼 {formatDistance(selectedPoi.distance)} · 高德评分 {selectedPoi.rating||'暂无'}</p></div><div className="decision risk"><h3>待补充信息</h3><p>品牌联系人、所需面积、租金承受、预计营收与工程条件。</p></div></div></div>
