...
export default function Workbench() {
  ...
  useEffect(()=>{
    ...
    fetch(`/api/feishu?action=records&year=${merchantYear}`,{cache:'no-store'}).then(async response=>{const data=await response.json();if(!response.ok||!data.ok) throw new Error(data.message);return data;}).then(data=>{if(active){setFeishuRecords(data.records||[]);setFeishuDataState('live');}}).catch(()=>{if(active)setFeishuDataState('fallback');});
    return ()=>{active=false;};
  },[merchantYear]);
  useEffect(()=>{
    let active=true;
  ...
  function MerchantsPage(){return <>{header(<button className="btn" onClick={()=>notify('商户数据已导出')}><Download size={16}/>导出 CSV</button>)}<div className="grid four" style={{marginBottom:16}}>{CATEGORY_GROUPS.slice(1).map(label=>{const count=displayMerchants.filter(item=>item.group===label).length;return <MetricCard key={label} label={label} value={`${count} 家`} note={`占比 ${displayMerchants.length?Math.round(count/displayMerchants.length*100):0}%`}/>})}</div><section className="panel"><div className="toolbar"><label className="field search"><span className="field-label">搜索商户</span><span className="input-icon"><Search size={16}/><input className="form-control" value={merchantSearch} onChange={e=>setMerchantSearch(e.target.value)} placeholder="名称、大类或细分业态"/></span></label><label className="field"><span className="field-label">年份</span><select className="form-select" value={merchantYear} onChange={e=>setMerchantYear(e.target.value)}><option value="2025">2025年</option><option value="2026">2026年</option></select></label><label className="field"><span className="field-label">月份</span><select className="form-select" value={merchantMonth} onChange={e=>setMerchantMonth(e.target.value)}><option value="all">全年累计</option>{Array.from({length:12},(_,index)=><option key={index+1} value={String(index+1)}>{index+1}月</option>)}</select></label></div><div className="table-wrap"><table style={{minWidth:900}}><thead><tr><th>商户</th><th>大类</th><th>细分业态</th><th>楼层</th><th className="num">客单价</th><th className="num">面积</th><th className="num">日均客流</th><th className="num">{merchantMonth==='all'?`${merchantYear}年累计销售`:`${merchantMonth}月销售`}</th><th>状态</th></tr></thead><tbody>{filteredMerchants.map(item=><tr key={item.id}><td>{item.name}</td><td><span className={`category-swatch ${groupClass(item.group)}`}/>{item.group}</td><td>{item.subtype}</td><td>{item.floor||'未提供'}</td><td className="num">{item.spend==null?'未提供':`${item.spend}元`}</td><td className="num">{item.area==null?'未提供':`${money(item.area)}㎡`}</td><td className="num">{item.daily==null?'未提供':item.daily}</td><td className="num">{item.revenue==null?'未提供':`${money(item.revenue)}万`}</td><td><span className="badge">{item.status||'在营'}</span></td></tr>)}</tbody></table></div><p className="panel-note" style={{marginTop:12}}>显示 {filteredMerchants.length}/{displayMerchants.length} 家商户 · {merchantYear}年{merchantMonth==='all'?'全年':`${merchantMonth}月`} · {feishuDataState==='loading'?'读取中':'飞书只读'}</p></section></>}
