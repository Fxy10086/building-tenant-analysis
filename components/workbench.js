...
export default function Workbench() {
  ...
  const analysisUploadPanel = () => <div className={`analysis-upload ${analysisUploadState==='error'?'error':''}`}><div className="analysis-upload-copy"><FileSpreadsheet size={19}/><div><strong>补充分析资料</strong><span>{analysisUploadState==='ready'?`${analysisSupplement.name} · 已生成报告`:analysisUploadState==='reading'?'正在校验 Excel...':'先下载模板，填写黄色列后上传'}</span></div></div><div className="analysis-upload-actions"><a className="btn" href="/templates/候选商户补充分析资料模板.xlsx" download><Download size={15}/>下载模板</a><button className="btn primary" onClick={()=>analysisFileInput.current?.click()} disabled={analysisUploadState==='reading'}><Upload size={15}/>{analysisUploadState==='reading'?'读取中':'上传 Excel'}</button>{analysisSupplement&&<button className="icon-btn danger" onClick={clearAnalysisSupplement} aria-label="移除上传资料" title="移除上传资料"><Trash2 size={15}/></button>}<input ref={analysisFileInput} type="file" accept=".xlsx,application/vnd.openxmlformats-officedocument.spreadsheetml.sheet" onChange={uploadAnalysisWorkbook} style={{display:'none'}}/></div>{analysisUploadError&&<p>{analysisUploadError}</p>}<small>文件仅在当前浏览器中解析，不上传服务器、不回写飞书。</small></div>;
