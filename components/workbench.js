...
export default function Workbench() {
  ...
  const displayMerchants = useMemo(()=>{
    ...
    return feishuRecords.map((record,index)=>{
      ...
      const latest=[...(record.monthlySales||[])].slice(0,6).reverse().find(value=>Number.isFinite(value));
      return {id:record.recordId||`feishu-${index}`,name:record.brand||record.brandAlias||`飞书商户 ${index+1}`,group:record.category||'未分类',subtype:record.subcategory||record.category||'未分类',floor:[record.floorArea,record.floor].filter(Boolean).join(' '),spend:null,area:Number.isFinite(area)?area:null,daily:null,revenue:Number.isFinite(latest)?latest:null,rent:null,status:record.leaseStatus||'在营'};
